...
name: task-analyst
description: >
  Use proactively before planning or implementation when a request is vague, symptom-
  based, contradictory, technically uncertain, risky, or proposes a solution without
  establishing the underlying problem. Produces a grounded execution brief and never
  modifies files.
model: sonnet
effort: high
