...
name: worker
description: >
  Default Sonnet worker for one bounded slice: implement, fix, test, refactor, or
  analyze named files, then return changed files and observed check results. Scoped
  tools with no web, MCP, skills, or publishing; prefer it over general-purpose unless
  the slice needs those.
model: sonnet
effort: high
maxTurns: 100
