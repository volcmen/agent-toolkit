---
name: agent-board
description: Act as the product manager for a local autonomous Kanban of coding agents, driven by the `ab` CLI. Use when the user states goals or a wish list ("do a, b, c", "build X", "put this on the board", "plan this out", "work through these"), or asks to check, unblock, re-route, re-spec, attach to, or report on in-flight agent cards. Cards are markdown files, roles are souls, triage is a cheap local model, and every run is budget-capped. Also use when the user asks what the agents are doing or what the board spent.
---
...
Binary: `bun run <repo>/bin/ab.ts` (or `ab` if linked). Board lives in `board/`
next to the repo it works on; `ab where` shows which one is active and why.
...
## 2. Intake — let triage do the writing

For anything vague, add it raw and let the cheap model spec + split + route it:

```bash
ab add "add a /health endpoint to the demo service and document it"
ab triage                     # or wait for the next dispatcher tick
```
...
When you already know the shape, skip triage by naming the role:
...
Before a big fan-out, look at `ab stats`. SQLite atomically reserves configured
worst-case metered spend before launch; a failed reservation parks/skips the card
(`budget.perCardUsd`, `budget.perDayUsd`, `budget.perRunReserveUsd`). This is not
a provider-side stop guarantee for subscription or unknown-price runtimes.
...
- Claude cards are for a genuinely different perspective (adversarial review,
  ambiguous architecture). Keep their `--budget` low and expect to fall back
...
- Never raise a budget to make a stuck card pass. Re-spec or split it.
...
spend real budget on that thinness.
...
with no further input, and current spend against the caps. Never claim a card is
